...
class Iogen(Executor):
    ...
    def fetch_description(self):
        url = 'https://www.iogen.com/about-us/'
        response = requests.get(url)
        soup = BeautifulSoup(response.content, 'html.parser')

        section = soup.find('div', class_='text-block')  # adjust class as per site
        if section:
            p = section.find('p')
            if p:
    ...
    def fetch_clients(self):
        # Iogen may not have client logos on homepage, can extract partners if available
        url = "https://www.iogen.com/our-partners/"
        response = requests.get(url)
    ...
    def get_news_details(self):
        returnres = ""
        url = "https://www.iogen.com/news/"  # latest news page
        returnres += f"url: {url}\n\n"
        response = requests.get(url)
        soup = BeautifulSoup(response.text, "html.parser")
